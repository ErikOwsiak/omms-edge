...
class ttyUSBDeviceScanner(_th.Thread):
   ...
   def init(self):
      ...
      self.redops.update_edge_diag(self.diag_tag, mapdct=_d, restart=True)
   ...
   def run(self):
      ...
      self.redops.update_edge_diag(self.diag_tag, mapdct=_d)
      print("\n\t-- [ pinging_end ] --\n")
   ...
   def __create_dev_aliases(self):
      ...
      for d, a, m in self.located_map:
         ...
         # -- -- -- -- --
         print([b0, b1])
         self.redops.update_edge_diag(self.diag_tag, mapdct={d: b1})
